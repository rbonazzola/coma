...
def vertex_quadrics(mesh):
    ...
    # Allocate quadrics
    v_quadrics = np.zeros((len(mesh.triangles), 4, 4,))

    # For each face...
    for f_idx in range(len(mesh.triangles)):

        # Compute normalized plane equation for that face
        vert_idxs = mesh.triangles[f_idx]
        from IPython import embed; embed()
        verts = np.hstack((mesh.triangles[vert_idxs], np.array([1, 1, 1]).reshape(-1, 1)))
        ...
        for k in range(3):
            v_quadrics[mesh.points[f_idx, k], :, :] += np.outer(eq, eq)
...
def qslim_decimator_transformer(mesh, factor=None, n_verts_desired=None):
    ...
    if n_verts_desired is None:
        n_verts_desired = math.ceil(len(mesh.triangles) * factor)
    ...
    vert_adj = get_vertices_per_edge(mesh.triangles, mesh.points)
    # vert_adj = sp.lil_matrix((len(mesh.triangles), len(mesh.triangles)))
    # for f_idx in range(len(mesh.points)):
    #     vert_adj[mesh.points[f_idx], mesh.points[f_idx]] = 1

    vert_adj = sp.csc_matrix((vert_adj[:, 0] * 0 + 1, (vert_adj[:, 0], vert_adj[:, 1])), shape=(len(mesh.triangles), len(mesh.triangles)))
    ...
    for k in range(vert_adj.nnz):
        ...
        cost = collapse_cost(Qv, r, c, mesh.triangles)['collapse_cost']
        heapq.heappush(queue, (cost, (r, c)))

    # decimate
    collapse_list = []
    nverts_total = len(mesh.triangles)
    faces = mesh.points.copy()
    while nverts_total > n_verts_desired:
        ...
        cost = collapse_cost(Qv, r, c, mesh.triangles)
    ...
    new_faces, mtx = _get_sparse_transform(faces, len(mesh.triangles))
...
def generate_transform_matrices(mesh, factors):
    ...
    M,A,D,U = [], [], [], []
    A.append(get_vert_connectivity(mesh.points, np.array(mesh.triangles)))
    M.append(mesh)
    ...
    for factor in factors:
        ...
        M.append(new_mesh)
        A.append(get_vert_connectivity(new_mesh.triangles, new_mesh.points))
